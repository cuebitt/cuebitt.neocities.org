...
export function loadPersistentValues() {
	const data =
		JSON.parse(window.localStorage.getItem("wikimakerData")) || defaultData;

	return {
		...data,
		card: {
			...data.card,
			details: data.card.details.map((detail) => ({ ...detail, id: nanoid() })),
		},
	};
}
...
export function savePersistentValues(data) {
	const dataRemovedId = {
		...
		card: {
			...data.card,
			details: data.card.details.map((detail) => ({
				label: detail.label,
				value: detail.value,
			})),
		},
