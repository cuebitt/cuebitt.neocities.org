...
function constructTextSection(header, content, id) {
	const sectionElem = document
		.getElementById("body-text-section-template")
	...
	sectionElem.querySelector(".section-body").innerHTML = DOMPurify.sanitize(
		micromark(content, { plugins: [gfm()], htmlExtensions: [gfmHtml] }),
	);
	for (a of sectionElem.querySelectorAll(".section-body a")) {
		a.target = "_blank";
...
// image:
// {src: 'https://placehold.co/200', alt: 'Placeholder image', artist: 'artistname', date: ''}
function constructSingleImageSection(header, image, id) {
	// eslint-disable-line no-unused-vars
}
	...
	for (const section of mainSections) {
		...
		switch (section.type) {
			case "text":
				sections.set(
					id,
					constructTextSection(section.heading, section.content, id),
				);
				break;
			default:
